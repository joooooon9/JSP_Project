...
function DoDelete()
{
	if(confirm("지금 글을 정보를 삭제하시겠습니까?") == false)
	{
		return;	
	}
...
<span style="color: black; font-weight: bold;">▶자유게시판 글보기</span><br><br><br>
                <!-- 자유게시판 글보기 부분 -->
                <table border="1" style="margin:auto; width: 1000px; border-spacing: 0px; border-collapse : collapse;">
                    <tr>
                ...
                <form style="text-align: center; font-size: 30px;">
                        ...
                        <button type="button" onclick="location.href='free_list.jsp?kind=<%= vo.getBkind() %>&tag=<%= tag %>&keyword=<%=en_keyword %>&page=<%= pageno %>'">글목록</button>
                        <%
                        	if(login != null && login.getUserno().equals(vo.getUserno()))
                        	{
                        %>
						<button type="button" onclick="location.href='free_modify.jsp?kind=F&tag=<%= tag %>&keyword=<%=en_keyword %>&page=<%= pageno %>&no=<%= vo.getBno()%>'">글수정</button>
