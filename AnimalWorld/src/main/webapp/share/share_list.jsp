...
.now
{
  ...
  <span style="color: black; font-weight: bold;">▶나눔게시판 글목록</span><br><br><br>
  <table border="0" class="" style="margin: auto; width: 1000px; border-spacing: 0px;">
    ...
    //출력 번호 계산 = (전체 갯수) - (페이지번호 - 1) * 10
      ...
      <tr style="background-color:#f4f4f4;">
          ...
          <% 
		        String thead = item.getThead();
		        String color = "black";  // 기본 색상
		        if (thead.equals("나눔전"))
		        {
		            color = "blue";
		        } else if (thead.equals("나눔완료")) 
		        {
		            color = "red";
		        }else
		        {
		    ...
		    %>
          <span style="color: <%= color %>;">[<%= thead %>]</span><a href="share_view.jsp?kind=<%= kind %>&tag=<%= tag %>&keyword=<%= en_keyword %>&page=<%= pageno %>&no=<%=item.getBno()%>"><%= item.getBtitle() %></a>
  	...
  	{
  %>
  <p><button type="button" style="width: 95px; height: 30px;" onclick="location.href='share_write.jsp'">글쓰기</button></p>
  <%
  	}else
  	{
  		%>
<p><input type="button" style="width: 95px; height: 30px;" onclick="check()" value="글쓰기"></p> 							              		
  ...
  %>    	
		...
		{
			%>
			<a href="share_list.jsp?kind=<%= kind %>&tag=<%= tag %>&keyword=<%= en_keyword %>&page=<%= startBlock - 1 %>">이전</a>
			<%		
		}
		for(int p = startBlock; p <= endBlock; p++)
		{
			...
			if(p == pageno) css = "now";
			%>
			<a class="<%= css %>" href="share_list.jsp?kind=<%= kind %>&tag=<%= tag %>&keyword=<%= en_keyword %>&page=<%= p %>"><%= p %></a>
			<%
		}
		if(endBlock != maxpage)
		{
			%>
			<a href="share_list.jsp?kind=<%= kind %>&tag=<%= tag %>&keyword=<%= en_keyword %>&page=<%= endBlock + 1 %>">다음</a>
			<%
		}	
		%>
		</td>
  ...
  <table  border="0" class="pull-right" style="margin-left:auto;margin-right:auto; width: 1200px;">
       <tr>
           <td>
               <form method="get" name="search" action="share_list.jsp" style="text-align: center;" accept-charset="utf-8">
