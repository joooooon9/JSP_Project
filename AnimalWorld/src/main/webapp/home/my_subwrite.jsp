...
ArrayList<BoardVO> list = dto.MyList(pageno,userno,"F",tag,keyword);
...
<table border="0" style="width: 100%;">
                    <tr>
                        <td>
                            <span style="color: black; font-weight: bold;">▶ 임시저장 글</span><br><br>
                        </td>
                    </tr>
                </table>
                <table border="0" style="width: 100%; ">
    ...
    //출력 번호 계산 = (전체 갯수) - (페이지번호 - 1) * 10
      ...
      %>
		...
		{
			%>
			<a href="my_subwrite.jsp?tag=<%= tag %>&keyword=<%=en_keyword %>&kind=<%= kind %>&page=<%= startBlock - 1 %>">이전</a>
			<%		
		}
		for(int p = startBlock; p <= endBlock; p++)
		{
			...
			if(p == pageno) css = "now";
			%>
			<a class="<%= css %>" href="my_subwrite.jsp?tag=<%= tag %>&keyword=<%=en_keyword %>&kind=<%= kind %>&page=<%= p %>"><%= p %></a>
			<%
		}
		if(endBlock != maxpage)
		{
			%>
			<a href="my_subwrite.jsp?tag=<%= tag %>&keyword=<%=en_keyword %>&kind=<%= kind %>&page=<%= endBlock + 1 %>">다음</a>
			<%
		}	
		%>
		</td>
  ...
  <table  border="0" class="pull-right" style="margin-left:auto;margin-right:auto; width: 1200px;">
      <tr>
          <td>
              <form method="get" name="search" action="my_subwrite.jsp" style="text-align: center;" accept-charset="utf-8">
