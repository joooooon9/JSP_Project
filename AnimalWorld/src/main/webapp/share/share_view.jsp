...
<span style="color: black; font-weight: bold;">▶나눔게시판 글보기</span><br><br><br>
<!-- 자유게시판 글보기 부분 -->
...
<form style="text-align: center; font-size: 30px;">
        <button type="button" onclick="location.href='share_list.jsp?kind=<%= vo.getBkind() %>&tag=<%= tag %>&keyword=<%= en_keyword %>&page=<%= pageno %>'">글목록</button>
        <%
        	if(login != null && login.getUserno().equals(vo.getUserno()))
        	{
        %>
<button type="button" onclick="location.href='share_modify.jsp?kind=S&page=<%= pageno %>&tag=<%= tag %>&keyword=<%= en_keyword %>&no=<%= vo.getBno()%>'">글수정</button>
