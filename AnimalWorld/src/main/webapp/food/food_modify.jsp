...
<script>
	...
	<span style="color: black; font-weight: bold;">▶식품추천게시판 글수정</span><br><br><br>
	<form action="food_modifyok.jsp" method="post" id="modiok" enctype="multipart/form-data">
	...
	<p style="color: red; margin-left: 249px">[ 알림 ] 식품추천 게시판에서 욕설 또는 비방글 등은 사전 경고 없이 관리자에 의하여 삭제 됩니다.</p>
		...
		<input type="hidden" value="T" name="saveFlag" id="saveFlag">
		    ...
		    <button type="button" class="my_btn" onclick="location.href= 'food_view.jsp?kind=R&page=<%= pageno %>&no=<%= vo.getBno()%> '">취소</button>
	    </div>
	</form>
